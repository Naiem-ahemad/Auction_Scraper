...
from database.url_database import COUNTY_URLS
...
def scrape_county(county_name, base_url):
    ...
    try:
        ...
        print(f"[✗] Scrape failed for {county_name}: {e}")
    finally:
        driver.quit()
...
def main():
    ...
    if not os.path.exists(availability_file):
        print(f"[!] Missing availability file: '{availability_file}'")
        return
    ...
    for county, url in COUNTY_URLS.items():
        ...
        if not match.empty:
            ...
            if available:
                ...
                print(f"[-] Skipping {county} (not available)")
        else:
            print(f"[!] No match for {county}")

    try:
        os.remove(availability_file)
        print(f"[✓] Deleted availability file '{availability_file}'")
    except Exception as e:
        print(f"[!] Could not delete availability file: {e}")
